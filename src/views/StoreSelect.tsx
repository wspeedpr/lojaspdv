...
export default function StoreSelect({ user, onSelect, onLogout }: StoreSelectProps) {
  // Filtra lojas que o usuário tem acesso
  const availableStores = STORES.filter(s => user.storeIds.includes(s.id))
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-600 text-xs mt-1">
            {availableStores.length === 1
              ? 'Você tem acesso a 1 loja'
              : `Você tem acesso a ${availableStores.length} lojas · Cada loja possui banco de dados isolado`}
          </p>
        </div>

        {availableStores.length === 0 ? (
          <div className="bg-[#111118] border border-white/8 rounded-2xl p-8 text-center">
            <p className="text-slate-400 text-sm">Nenhuma loja atribuída à sua conta.</p>
            <p className="text-slate-600 text-xs mt-1">Contate o administrador do sistema.</p>
          </div>
        ) : (
