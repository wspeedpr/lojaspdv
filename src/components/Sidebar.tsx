...
export type View = 'dashboard' | 'pdv' | 'faturas' | 'retaguarda' | 'funcionarios' | 'crm' | 'comissoes' | 'sgp' | 'config'
    ...
    <aside className="w-64 min-h-screen bg-[#0D0D14] border-r border-white/8 flex flex-col flex-shrink-0">
      ...
      <div className="px-5 py-5 border-b border-white/8">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-white text-lg tracking-tight">NexoPDV</span>
        </div>
      </div>
      ...
      <div className="px-3 pb-5 space-y-1 border-t border-white/8 pt-4">
        <div className="px-3 py-2">
          ...
          <p className="text-slate-600 text-xs">{user.role === 'superadmin' ? 'Super Administrador' : user.role === 'admin' ? 'Administrador' : 'Gerente'}</p>
        </div>
        <button
          onClick={onChangeStore}
